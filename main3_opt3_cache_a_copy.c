...
int kernel_gblur(int argc, char **argv) {
    ...
    hb_mc_device_foreach_pod_id(&device, pod)
    {
        ...
        short int * A_copy_gray_host = (short int *) malloc(sizeof(short int) * SIZE);
        ...
        for (int i = 0; i < H; i++) {
            for (int j = 0; j < H; j++) {
                ...
                B_host[idx] = 0;
                A_copy_gray_host[idx] = A_gray_host[idx];
            }
        }
        for (int i = 1; i < H-1; i++) {
            for (int j = 1; j < H-1; j++) {
...
#define POD_CACHE_ALIGNED
    ...
    BSG_CUDA_CALL(hb_mc_device_malloc(&device, align_size*sizeof(int)*CACHE_LINE_WORDS, &temp_device2));
        ...
        hb_mc_dma_htod_t htod_job [] = {
            ...
            {
            .d_addr = A_copy_device,
            .h_addr = (void *) &A_copy_gray_host[0],
            .size = SIZE * sizeof(short int)
            },
            {
            .d_addr = B_device,
        ...
        #define CUDA_ARGC 4
        uint32_t cuda_argv[CUDA_ARGC] = {A_device, A_copy_device, B_device, H};
