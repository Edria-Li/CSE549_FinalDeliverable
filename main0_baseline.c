    ...
    hb_mc_device_foreach_pod_id(&device, pod)
    {
        ...
        for (int i = 0; i < H; i++) {
            for (int j = 0; j < H; j++) {
                ...
                if (i > 0   && j > 0  ) B_expected_host[idx] += kernel[0] * A_gray_host[idx-H-1];
                if (i > 0             ) B_expected_host[idx] += kernel[1] * A_gray_host[idx-1  ];
                if (i > 0   && j < H-1) B_expected_host[idx] += kernel[2] * A_gray_host[idx+H-1];
                if (j > 0             ) B_expected_host[idx] += kernel[3] * A_gray_host[idx-H  ];
                                        B_expected_host[idx] += kernel[4] * A_gray_host[idx    ];
                if (j < H-1           ) B_expected_host[idx] += kernel[5] * A_gray_host[idx+H  ];
                if (i < H-1 && j > 0  ) B_expected_host[idx] += kernel[6] * A_gray_host[idx-H+1];
                if (i < H-1           ) B_expected_host[idx] += kernel[7] * A_gray_host[idx+1  ];
                if (i < H-1 && j < H-1) B_expected_host[idx] += kernel[8] * A_gray_host[idx+H+1];
                B_host[idx] = 0;
            }
        }
    ...
    BSG_CUDA_CALL(hb_mc_device_malloc(&device, align_size*sizeof(int)*CACHE_LINE_WORDS, &temp_device2));
        ...
        for (int i = 0; i < SIZE; i++) {
            if (B_host[i] != B_expected_host[i]) {
                ...
                printf("FAIL [%d]: expected=%d, actual=%d, A_gray[i]=%d\n", i, B_expected_host[i], B_host[i], A_gray_host[i]);
